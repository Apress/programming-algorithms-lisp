...
(deftest queue ()
  (let ((q (make-queue)))
    (should be equalp (read-file-into-string "#S(QUEUE :HEAD NIL :TAIL NIL)")
            q)
    (enqueue 1 q)
    (enqueue 2 q)
    (enqueue 3 q)
    (should be equalp (read-file-into-string "#S(QUEUE :HEAD (3 2 1) :TAIL NIL)")
            q)
    (dequeue q)
    (should be equalp (read-file-into-string "#S(QUEUE :HEAD NIL :TAIL (2 3))")
            q)
    (enqueue 4 q)
    (should be equalp (read-file-into-string "#S(QUEUE :HEAD NIL :TAIL (2 3))")
            q)
    (dequeue q)
    (should be equalp (read-file-into-string "#S(QUEUE :HEAD (4) :TAIL (2 3))")
            q)
    (dequeue q)
    (should be equalp (read-file-into-string "#S(QUEUE :HEAD (4) :TAIL (3))")
            q)
    (dequeue q)
    (should be equalp (read-file-into-string "#S(QUEUE :HEAD (4) :TAIL NIL)")
            q)
    (dequeue q)
    (should be equalp (read-file-into-string "#S(QUEUE :HEAD NIL :TAIL NIL)")
            q)))
